...
import { Bell, CheckCheck, Loader2, Folder, DollarSign, CalendarDays, FileText, ExternalLink, User, UserCheck } from 'lucide-react';
...
// Assuming SharedData is defined globally or imported from a types file
// For example:
...
export function AppSidebarHeader({
    ...
    const { auth } = usePage<SharedData>().props; // Using 'any' for SharedData as it's not defined in the provided context

    return (
        <>
            {auth.is_impersonating && (
                <div className="bg-amber-500 text-white px-4 py-2 flex items-center justify-between text-xs font-medium">
            ...
            <header className="flex h-16 shrink-0 items-center justify-between gap-2 px-4 transition-[width,height] ease-linear group-has-[[data-collapsible=icon]]/sidebar-wrapper:h-12 border-b md:px-4 bg-[var(--sidebar)] text-white md:bg-transparent md:text-foreground border-white/20 md:border-sidebar-border/50">
                <div className="flex items-center gap-2 flex-1 text-inherit">
                    <SidebarTrigger className="-ml-1 text-inherit" />
